    ...
    public class ThreadManager
    {
        ...
        public async Task<Result> CreateNewThreadAsync(NewThread newThreadEntity, CancellationToken token = default)
        {
            ...
            using var form = new MultipartFormDataContent
            {
                ...
                { new StringContent(newThreadEntity.PostIcon?.Id.ToString(CultureInfo.InvariantCulture) ?? string.Empty), "iconid" },
                { new StringContent(newThreadEntity.Subject?.HtmlEncode() ?? string.Empty), "subject" },
                { new StringContent(newThreadEntity.Content?.HtmlEncode() ?? string.Empty), "message" },
                { new StringContent(newThreadEntity.ParseUrl.ToString() ?? string.Empty), "parseurl" },
