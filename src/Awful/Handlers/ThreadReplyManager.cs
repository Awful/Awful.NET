...
namespace Awful.Managers
{
    ...
    public class ThreadReplyManager
    {
        ...
        public async Task<ThreadReply> GetReplyCookiesForEditAsync(int postId, CancellationToken token = default)
        {
            if (postId <= 0)
            {
                throw new FormatException(Awful.Resources.ExceptionMessages.PostIdMissing);
            }
        ...
        public async Task<ThreadReply> GetReplyCookiesAsync(int threadId = 0, int postId = 0, CancellationToken token = default)
        {
            ...
            {
                throw new FormatException(Awful.Resources.ExceptionMessages.ThreadAndPostIdMissing);
            }
            ...
            try
            {
                ...
                {
                    throw new Exceptions.AwfulParserException("Failed to find form infor while getting forum reply cookies page.", new Awful.Entities.SAItem(result));
                }
        ...
        public async Task<string> CreatePreviewPostAsync(ThreadReply forumReplyEntity, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                if (postbody == null)
                {
                    throw new Exceptions.AwfulParserException("Failed to find post body while getting forum preview page.", new Awful.Entities.SAItem(result));
                }
        ...
        public async Task<string> GetQuoteStringAsync(int postId, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                if (quoteString == null)
                {
                    throw new Exceptions.AwfulParserException("Failed to find quote string body while getting quote string page.", new Awful.Entities.SAItem(result));
                }
